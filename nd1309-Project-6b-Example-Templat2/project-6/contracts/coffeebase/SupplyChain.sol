...
contract SupplyChain is Ownable, ConsumerRole, DistributorRole, FarmerRole, RetailerRole{
  ...
  function harvestItem(uint _upc, address _originFarmerID, string memory _originFarmName, string memory _originFarmInformation, string  memory _originFarmLatitude, string  memory _originFarmLongitude, string  memory _productNotes) onlyFarmer() public 
  {
    ...
    items[_upc] = Item(sku, _upc, _originFarmerID, _originFarmerID, _originFarmName, _originFarmInformation, _originFarmLatitude, _originFarmLongitude, sku+_upc, _productNotes, 0 ,State.Harvested, address(0), address(0), address(0));
    // Increment sku
    sku = sku + 1;
  ...
  function processItem(uint _upc) harvested(_upc) verifyCaller(msg.sender) onlyFarmer() public 
  ...
  function packItem(uint _upc) processed(_upc) verifyCaller(msg.sender) onlyFarmer() public 
  // Call modifier to check if upc has passed previous supply chain stage
  ...
  function sellItem(uint _upc, uint _price) packed(_upc) verifyCaller(msg.sender) onlyFarmer() public
  // Call modifier to check if upc has passed previous supply chain stage
  ...
  function shipItem(uint _upc) sold(_upc) verifyCaller(msg.sender) onlyDistributor() public 
    // Call modifier to check if upc has passed previous supply chain stage
